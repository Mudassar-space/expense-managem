...
from apis import models, oaut2, schemas
...
@app.post('/login', tags=["Authentication"], responses={status.HTTP_200_OK: {"model": schemas.Responses},
...
def login(request: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    return {"access_token": "09d25e094faa6ca2556c818166b7a9563b93f7099f6f0f4caa6cf63b88e8d3e7", "token_type": "bearer"}
...
@app.post('/categories', tags=['categories'], status_code=status.HTTP_201_CREATED, response_model=schemas.Cate_response_model,
    ...
    status.HTTP_422_UNPROCESSABLE_ENTITY: {"model":  schemas.Responses},
    status.HTTP_500_INTERNAL_SERVER_ERROR: {"model":  schemas.Responses}})
def create(request: schemas.Categories, db: Session = Depends(get_db), currrent_user: schemas.User = Depends(oaut2.get_current_user)):
    new_category = models.CategoriesTableSchema(
        created_on=request.created_on, name=request.name, user_id=request.user_id)
    try:
        db.add(new_category)
        db.commit()
...
def show(Id: int, db: Session = Depends(get_db), currrent_user: schemas.User = Depends(oaut2.get_current_user)):
    category = db.query(models.CategoriesTableSchema).filter(
        models.CategoriesTableSchema.category_Id == Id).first()
    if not category:
...
@app.get('/categories', tags=['categories'], responses={status.HTTP_200_OK: {"model": List[schemas.Cate_response_model]},
                                                        ...
                                                        status.HTTP_422_UNPROCESSABLE_ENTITY: {"model":  schemas.Responses},
                                                        status.HTTP_500_INTERNAL_SERVER_ERROR: {"model":  schemas.Responses}})
def all(db: Session = Depends(get_db), currrent_user: schemas.User = Depends(oaut2.get_current_user)):
...
@app.put('/categories/{id}', tags=['categories'], responses={status.HTTP_200_OK: {"model": schemas.Categories},
                                                             ...
                                                             status.HTTP_422_UNPROCESSABLE_ENTITY: {"model":  schemas.Responses},
                                                             status.HTTP_500_INTERNAL_SERVER_ERROR: {"model":  schemas.Responses}})
def update(id: str, ques: schemas.Categories, db: Session = Depends(get_db), currrent_user: schemas.User = Depends(oaut2.get_current_user)):
    data = ques.dict()
    result = db.query(models.CategoriesTableSchema).filter(
        models.CategoriesTableSchema.category_Id == id).update(data)
    db.commit()
    if not result:
        return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content=jsonable_encoder(schemas.Responses(status=False, message="Not found")))
    try:
...
@app.delete('/categories/{id}', tags=['categories'], status_code=status.HTTP_204_NO_CONTENT, responses={
    ...
    status.HTTP_422_UNPROCESSABLE_ENTITY: {"model": schemas.Responses},
    status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": schemas.Responses}})
def Destroy(id, db: Session = Depends(get_db), currrent_user: schemas.User = Depends(oaut2.get_current_user)):
    deleted = db.query(models.CategoriesTableSchema).filter(
        models.CategoriesTableSchema.category_Id == id).delete(synchronize_session=False)
    db.commit()
    if not deleted:
        return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content=jsonable_encoder(schemas.Responses(status=False, message="Not found")))
    try:
...
@app.post('/users', status_code=201, tags=['users'], response_model=schemas.User_response, responses={
    ...
    status.HTTP_422_UNPROCESSABLE_ENTITY: {"model":  schemas.Responses},
    status.HTTP_500_INTERNAL_SERVER_ERROR: {"model":  schemas.Responses}})
def create(user: schemas.User, db: Session = Depends(get_db), currrent_user: schemas.User = Depends(oaut2.get_current_user)):
...
@app.put('/users/{id}', tags=['users'], responses={status.HTTP_200_OK: {"model": schemas.User},
                                                   ...
                                                   status.HTTP_422_UNPROCESSABLE_ENTITY: {"model":  schemas.Responses},
                                                   status.HTTP_500_INTERNAL_SERVER_ERROR: {"model":  schemas.Responses}})
def update(id: int, ques: schemas.User, db: Session = Depends(get_db), currrent_user: schemas.User = Depends(oaut2.get_current_user)):
    data = ques.dict()
...
@app.post('/accounts', status_code=201, tags=['accounts'],  response_model=schemas.Account_response,
    ...
    status.HTTP_422_UNPROCESSABLE_ENTITY: {"model":  schemas.Responses},
    status.HTTP_500_INTERNAL_SERVER_ERROR: {"model":  schemas.Responses}})
def create(account: schemas.Account, db: Session = Depends(get_db), currrent_user: schemas.User = Depends(oaut2.get_current_user)):
    new_account = models.Account(account_name=account.account_name, initial_balance=account.initial_balance,
                                 type=account.type, user_id=account.user_id, created_at=account.created_at)
    try:
        db.add(new_account)
        db.commit()
...
def show(Id: int, db: Session = Depends(get_db), currrent_user: schemas.User = Depends(oaut2.get_current_user)):
    account = db.query(models.Account).filter(
        models.Account.account_id == Id).first()
    if not account:
        return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content=jsonable_encoder(schemas.Responses(status=False, message="Not found")))
    try:
        return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(account))
...
@app.put('/accounts{id}', tags=['accounts'], responses={status.HTTP_200_OK: {"model": schemas.Account},
                                                        ...
                                                        status.HTTP_422_UNPROCESSABLE_ENTITY: {"model":  schemas.Responses},
                                                        status.HTTP_500_INTERNAL_SERVER_ERROR: {"model":  schemas.Responses}})
def update(id: int, para: schemas.Account, db: Session = Depends(get_db), currrent_user: schemas.User = Depends(oaut2.get_current_user)):
    data = para.dict()
    account = db.query(models.Account).filter(
        models.Account.account_id == id).update(data)
    db.commit()
    if not account:
        return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content=jsonable_encoder(schemas.Responses(status=False, message="Not found")))
    try:
...
@app.delete('/accounts{id}', tags=['accounts'], status_code=status.HTTP_204_NO_CONTENT, responses={
    ...
    status.HTTP_422_UNPROCESSABLE_ENTITY: {"model": schemas.Responses},
    status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": schemas.Responses}})
def delete(id, db: Session = Depends(get_db), currrent_user: schemas.User = Depends(oaut2.get_current_user)):
    deleted = db.query(models.Account).filter(
        models.Account.account_id == id).delete(synchronize_session=False)
    db.commit()
    if not deleted:
        return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content=jsonable_encoder(schemas.Responses(status=False, message="Not found")))
    try:
...
@app.post('/ledgers', status_code=201, tags=['ledgers'],  response_model=schemas.Ledger,
    ...
    status.HTTP_401_UNAUTHORIZED: {"model": schemas.Responses},
    status.HTTP_500_INTERNAL_SERVER_ERROR: {"model":  schemas.Responses}})
def create(ledger: schemas.Ledger, db: Session = Depends(get_db), currrent_user: schemas.User = Depends(oaut2.get_current_user)):
    new_ledger = models.Ledger(amount=ledger.amount, account_id=ledger.account_id, category_Id=ledger.category_Id,
                               user_id=ledger.user_id, transaction=ledger.transaction, transfer_to=ledger.transfer_to, created_at=ledger.created_at)
    try:
        db.add(new_ledger)
        db.commit()
...
def show(Id: int, db: Session = Depends(get_db), currrent_user: schemas.User = Depends(oaut2.get_current_user)):
    ledger = db.query(models.Ledger).filter(
        models.Ledger.ledger_id == Id).first()
    if not ledger:
        return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content=jsonable_encoder(schemas.Responses(status=False, message="Not found")))
    try:
...
@app.put('/ledgers{id}', tags=['ledgers'], responses={status.HTTP_200_OK: {"model": schemas.Ledger},
                                                      ...
                                                      status.HTTP_422_UNPROCESSABLE_ENTITY: {"model":  schemas.Responses},
                                                      status.HTTP_500_INTERNAL_SERVER_ERROR: {"model":  schemas.Responses}})
def update(id: int, para: schemas.Ledger, db: Session = Depends(get_db), currrent_user: schemas.User = Depends(oaut2.get_current_user)):
    data = para.dict()
    ledger = db.query(models.Ledger).filter(
        models.Ledger.ledger_id == id).update(data)
    db.commit()
    if not ledger:
        return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content=jsonable_encoder(schemas.Responses(status=False, message="Not found")))
    try:
...
@app.delete('/ledgers{id}', tags=['ledgers'], status_code=status.HTTP_204_NO_CONTENT, responses={
    ...
    status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": schemas.Responses}})
def delete(id, db: Session = Depends(get_db), currrent_user: schemas.User = Depends(oaut2.get_current_user)):
    deleted = db.query(models.Ledger).filter(
        models.Ledger.ledger_id == id).delete(synchronize_session=False)
    db.commit()
    if not deleted:
        return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content=jsonable_encoder(schemas.Responses(status=False, message="Not found")))
    try:
